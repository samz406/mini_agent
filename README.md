...
| 08 | 插件机制 | 文件系统扫描、importlib 动态加载、生命周期钩子 |
| 09 | MCP 协议 | Model Context Protocol、Server/Client、工具发现 |
...
│   ├── ch08_plugin/         # 动态插件机制 + extended_reading.md
│   └── ch09_mcp/            # MCP 协议集成 + extended_reading.md
│
└── mini_agent/              # 🚀 完整生产包
    ...
    ├── tools/
    │   ├── base.py          # 工具 + 注册表抽象
    │   └── builtins.py      # 内置工具（计算器、文件、bash、记忆…）
    │   └── mcp_client.py    # MCP 服务器集成（MCPToolLoader）
    ├── memory/
...
# 挂载 MCP Server，动态扩展工具（需要 Node.js）
...
| `run_bash` | 执行 bash 命令并返回输出（最长 30 秒，可配置） |
